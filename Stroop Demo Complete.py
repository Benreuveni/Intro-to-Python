...
line = visual.Line(win, start=(-30.0, lineY), end=(30.0, lineY), # Chose arbitrary points to start and end line. Can be adjusted later.
    ori = 0, name = 'line', opacity = 1, depth = 0.0)

a = visual.TextStim(
    win=win, ori=0, name='A',
    text="A",
    font='Arial',
    pos=[-10.0, -10.0], height=1.0, wrapWidth=None,
    color='white', opacity=1,
    depth=0.0)

b = visual.TextStim(
    win=win, ori=0, name='B',
    text="B",
    font='Arial',
    pos=[10.0, -10.0], height=1.0, wrapWidth=None,
    color='white', opacity=1,
...
def doUserInteraction(stim, expectedKeys, timeout, soundFile):
    ...
    while(True):
        ...
        if stim.pos[1] <= break_line and len(keys) < 1:
