...
'''
 The value of x (or bob) will change with each iteration of the FOR loop. So on the first time around, x == 1
...
'''
 You might have had the notion that this can be used to iterate over trials and stimuli through indexing!
...
'''
 ...
 how to reference actual usable time!
...
'''
 ...
 Psychopy has a great way of capturing keyboard events! It is called event.getKeys()
...
'''
 Consider what happens to "keys" when the first key is pressed. HINT: consider using the len() function.
