...
'''
 ...
 OK, enough zoology. Let's get back to Functions. Functions are self-contained untis of code that perform operations.
 In this context, self-contained means that the function is almost like a separate file of code. It really only knows
 about things inside the function and unless told, will not impact anything outside of itself - unless told.
...
'''
 Notice how you get an error for the print line. This is because "a" was defined Inside a function and as such, it is not
 accessible outside that function. This is what it means to be "self contained". This also means that you can reuse
 variables inside a function that have already been defined outside of it since variables defined within a function
 do not leave it unless "return"ed(though I'd recommend you not).
...
def selfContainedFunction2():
    a = 1+3
    print a
    return a
#print selfContainedFunction2()
...
'''
 ...
 We then told it to "print" the function call which resulted in printing "a" since it was the only thing returned.
 Now you might be wondering what would happen if you added a "print" command to line 29. Go ahead and try.
...
def selfContainedFunction3():
    a = 1+3
    print a
    return a
#a = selfContainedFunction3()
#print a
...
b = 1
def selfContainedFunction4():
    print b
#selfContainedFunction4()
...
'''
 ...
 arguments, does something and returns a value to function 1 (since it is the one that called the function).
 Function 1 knows that it needs to return (to whatever called it) the output of the second function.
 We store the output of function 1 in a variable "e" and then print e.
 ...
 2) the correct response - in the form of a list (indexed correctly)
 ...
 indexed in order to pass as an argument), and then both prints, and returns a new variable that is either 0 for
 incorrect or 1 for correct.

 Hint: This will involve at least a while loop and a function.

 Don't forget: 1) to call the function at the end.
               2) to make sure your While loop (hint!) has an exit clause!
               3) to create a window.

 Feel free to use what you've learned in "If statements" BUT I really really recommend you do NOT cut and paste it.
 Instead a much better way to cement your understanding is to recode it all from nothing (peeking is allowed).
'''
...
'''
 ...
 Until now, we've been playing around with non-visual operations which is a little boring. Let's see how we draw an
 image to the window.
...
'''
 The "textStim", "imageStim", "correctSound" and "woohoo" are arbitrary names I gave to a particular object, in this case,
 an TextStim, ImageStim and SoundStim(s).
...
'''
 ...
 The reason it did not is because, while we told PsychoPy to AutoDraw the image
 (which means keep on drawing it every frame until I say otherwise), we did not tell PsychoPy to expose the "buffer" to us.
 Therefore, the image was being drawn "behind the scenes".
 ...
 Next, we may also want to play a sound. This is done with the objectName.play() command.
 Try and add a command to the above code that will also play a sound for us. The sound duration is currently set at 0.5s.
...
'''
 ...
 Find several (~5) images to use. Rename them to something simple.
